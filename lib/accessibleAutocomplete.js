(function() {
	...
	var accessibleAutocompleteDirective = ['$http', '$compile', '$templateCache', '$q', 'accessibleAutocomplete', 'accessibleAutocompleteKeyProcessor', '$timeout',
		...
		return {
			...
			link: function(scope, element, attrs, ctrl) {
				...
				scope.options.ariaAutocomplete = element.attr('aria-autocomplete'),
				scope.options.ariaRole = element.attr('role');

				var options = scope.options;

				if (!options.ariaAutocomplete) {
					options.ariaAutocomplete = (options.ariaRole == 'textbox') ? 'inline' : 'list';
					element.attr('aria-autocomplete', options.ariaAutocomplete);
				}

				if (!attrs.role) {
					options.ariaRole = (options.ariaAutocomplete == 'list' || options.ariaAutocomplete == 'both')
						? 'combobox' : 'textbox';

					element.attr('role', options.ariaRole);
				}
				...
				if (options.ariaAutocomplete == 'list' || options.ariaAutocomplete == 'both') {
				...
				scope.$watch(function() {
					...
					if (options.ariaAutocomplete == 'inline' || options.ariaAutocomplete == 'both') {
	...
	var accessibleAutocompleteController = ['$scope', 'accessibleSerialTimeout', 'accessibleSerialPromise', 'accessibleAutocomplete',
		...
		$scope.isInlineCompletionEnabled = function() {
			return options.ariaAutocomplete == 'inline' || options.ariaAutocomplete == 'both';
		}
		...
		$scope.textChanged = function(newText) {
			...
			serialPromise(serialTimeout(null, $scope.options.delay)
				.then(function() {
				...
				.then(function(data) {
					...
					// TODO: Data formats
					$scope.suggestions = Array.isArray(data) ? data : [];
					$scope.isLoadingSuggestions = false;
